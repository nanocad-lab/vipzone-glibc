...
static void* sYSVIPMALLOc(INTERNAL_SIZE_T nb, INTERNAL_SIZE_T vf, mstate av)
{
    ...
    //if(vf==2) mSTATs();
    if(vf==1)
    printf("sYSVIPMALLOc: req[%lu,%lu], new size = %lu, T=%lu & [%lu < %lu] \n",
           (long unsigned int)nb,(long unsigned int)vf,
    ...
    if (
	    ...
	    long unsigned flags = MAP_PRIVATE | _VIP_F_READ;// | vf; (MAP_PRIVATE | _VIP_F_READ)|MAP_ANONYMOUS
            ...
            if (mm != MAP_FAILED) {
                ...
                if(vf==1)
                    printf("sYSVIPMALLOc: allocated [%lu @ %lu]\n",
                           (long unsigned int)size, (long unsigned int)mem);
    ...
    if(vf==1)
        printf("sYSVIPMALLOc: req[%lu,%lu], trying regular malloc() \n",
               (long unsigned int)nb,(long unsigned int)vf);
    
    return _int_malloc(av, nb);
}
...
public_vIPMALLOc(size_t bytes, size_t v_flags)
{
    ...
    {
        printf("public_vIPMALLOc: successful call to _int_vip_malloc(%lu @ %lu) \n",
               (long unsigned int)bytes,(long unsigned int)victim);
