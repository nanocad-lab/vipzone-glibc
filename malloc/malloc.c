...
static void*   vip_malloc_check(size_t sz, size_t vip_flags, const void *caller); //vipzone
...
static void*   vip_malloc_atfork(size_t sz, size_t vip_flags, const void *caller); //vipzone
...
/*
  sysvipmalloc handles vip_malloc cases requiring more memory from the system.
  On entry, it is assumed that av->top does not have enough
  space to service request for nb bytes, thus requiring that av->top
  be extended or replaced.
*/
//vipzone
static void* sYSVIPMALLOc(INTERNAL_SIZE_T nb, INTERNAL_SIZE_T vip_flags, mstate av)
{
    long            size;           /* arg to first MORECORE or mmap call */
    long            correction;     /* arg to 2nd MORECORE call */
    mchunkptr       p;              /* the allocated/returned chunk */
    ...
    //if(vip_flags==2) mSTATs();
    printf("<glibc-vipzone> sYSVIPMALLOc(): Got request for %lu bytes, vip_flags = %lu, new size = %lu, mmap_threshold = %lu, num mmaps = %lu, max num mmaps = %lu\n",
           (long unsigned int)nb,(long unsigned int)vip_flags,
           (long unsigned int)((nb + SIZE_SZ + pagemask) & ~pagemask),
           (unsigned long int)(mp_.mmap_threshold),
           (unsigned long int)mp_.n_mmaps, 
           (unsigned long int)mp_.n_mmaps_max);

    //if ((unsigned long)(nb) >= (unsigned long)(mp_.mmap_threshold) &&
    if (mp_.n_mmaps < mp_.n_mmaps_max) {
        char* mm;             /* return value from mmap call*/
	    ...
	    long unsigned flags = MAP_PRIVATE | MAP_ANONYMOUS; // | vip_flags; (MAP_PRIVATE | _VIP_F_READ)|MAP_ANONYMOUS
				printf("<glibc-vipzone> sYSVIPMALLOc(): vip_mmap syscall...\n");
            mm = (char*)(syscall(NR_vip_mmap, NULL, size, (PROT_READ|PROT_WRITE|vip_flags), flags, -1, 0));
            
            if (mm != MAP_FAILED) {
					printf("<glibc-vipzone> sYSVIPMALLOc(): vip_mmap syscall succeeded!\n");
					/*
					  The offset to the start of the mmapped region is stored
					  in the prev_size field of the chunk. This allows us to adjust
					  returned start address to meet alignment requirements here
					...
					printf("<glibc-vipzone> sYSVIPMALLOc(): vip_mmap syscall FAILED!\n");
		  }
		 return NULL;
 }
	
//vipzone: the following code was all commented out to make room for a simpler, Danny-based code
  ...
  */
	...
	printf("<vipzone-glibc> sYSVIPMALLOc(): We just got called. Can we use vip_mmap? nb = %lu, mmap_threshold = %lu, n_mmaps = %lu, n_mmaps_max = %lu\n", nb, mp_.mmap_threshold, mp_.n_mmaps, mp_.n_mmaps_max);
  if ((unsigned long)(nb) >= (unsigned long)(mp_.mmap_threshold) &&
      (mp_.n_mmaps < mp_.n_mmaps_max)) {
  ...
  try_mmap:
		printf("<vipzone-glibc> sYSVIPMALLOc(): at try_mmap...\n");
    /*
      Round up size to nearest page.  For mmapped chunks, the overhead
      is one SIZE_SZ unit larger than for normal chunks, because there
      is no following chunk whose prev_size field could be used.
    ...
    /* Don't try if size wraps around 0 */
    if ((unsigned long)(size) > (unsigned long)(nb)) {
		printf("<vipzone-glibc> sYSVIPMALLOc(): Making syscall to vip_mmap, spot #1\n");
      mm = (char*)(syscall(NR_vip_mmap, NULL, size, PROT_READ|PROT_WRITE|vip_flags, MAP_PRIVATE, -1, 0)); //vipzone

      if (mm != MAP_FAILED) {

			printf("<vipzone-glibc> sYSVIPMALLOc(): vip_mmap syscall, spot #1 success!\n");
	/*
	  The offset to the start of the mmapped region is stored
	  in the prev_size field of the chunk. This allows us to adjust
	  returned start address to meet alignment requirements here
	...
	printf("<vipzone-glibc> sYSVIPMALLOc(): we are right past try_mmap section...\n");
  /* Record incoming configuration of top */
  ...
  */
    ...
    if ((unsigned long)(size) > (unsigned long)(nb)) {
		...
		printf("<vipzone-glibc> sYSVIPMALLOc(): vip_mmap spot #2...\n");
      char *mbrk = (char*)(syscall(NR_vip_mmap, NULL, size, PROT_READ|PROT_WRITE|vip_flags, MAP_PRIVATE, -1, 0)); //vipzone

      if (mbrk != MAP_FAILED) {
			printf("<vipzone-glibc> sYSVIPMALLOc(): vip_mmap spot #2 success...\n");
	...
	*/
	...
	printf("<vipzone-glibc> sYSVIPMALLOc(): past vip_mmap spot #2...\n");
  if (brk != (char*)(MORECORE_FAILURE)) {
    if (mp_.sbrk_base == 0)
      mp_.sbrk_base = brk;
    av->system_mem += size;
    ...
    */
    ...
    if (brk == old_end && snd_brk == (char*)(MORECORE_FAILURE))
      ...
      /* Oops!  Someone else killed our space..  Can't touch anything.  */
      malloc_printerr (3, "break adjusted to free vip_malloc space", brk);
    }
...
public_vIPMALLOc(size_t bytes, size_t vip_flags)
{
    ...
    if (__builtin_expect (hook != NULL, 0))
        return (*hook)(bytes, vip_flags, RETURN_ADDRESS (0)); //vipzone
    ...
    victim = _int_vip_malloc(ar_ptr, bytes, vip_flags); //vipzone
    if(!victim) {
...
libc_hidden_def(public_vIPMALLOc)
...
/*
 ------------------------------ vipzone: _int_vip_malloc ------------------------------
 */

//vipzone: this is the core vip_malloc() implementation.
static void*
_int_vip_malloc(mstate av, size_t bytes, size_t vip_flags)
{
    ...
    */
	...
	printf("<glibc-vipzone> _int_vip_malloc: request for %lu bytes, internal size %lu bytes, vip_flags = %lu\n",
		(long unsigned int)bytes, (long unsigned) nb, (long unsigned int)vip_flags);

	void *p = sYSVIPMALLOc(nb, vip_flags, av);
	if (p != NULL && __builtin_expect (perturb_byte, 0))
		alloc_perturb (p, bytes);
	if (p == NULL)
		printf("<glibc-vipzone> _int_vip_malloc: sYSVIPMALLOc() returned NULL!\n");
	return p;

	//vipzone: the below was pre-process disabled as I'm trying a much simpler, Danny-based code above.
	...
	*/
	...
	if (victim == NULL)
	 ...
	 if (victim != 0) {
		if (__builtin_expect (fastbin_index (chunksize (victim)) != idx, 0))
	{
	  errstr = "vip_malloc(): memory corruption (fast)"; //vipzone
	errout:
	  malloc_printerr (check_action, errstr, chunk2mem (victim));
	  return NULL;
	}
	...
	*/
	...
	if (__builtin_expect (bck->fd != victim, 0))
	  {
		 errstr = "vip_malloc(): smallbin double linked list corrupted"; //vipzone
		 goto errout;
	  }
	set_inuse_bit_at_offset(victim, nb);
	bin->bk = bck;
	...
	*/
	...
	for(;;) {
	 ...
	 while ( (victim = unsorted_chunks(av)->bk) != unsorted_chunks(av)) {
		...
		if (__builtin_expect (victim->size <= 2 * SIZE_SZ, 0)
	  || __builtin_expect (victim->size > av->system_mem, 0))
	malloc_printerr (check_action, "vip_malloc(): memory corruption",
			 chunk2mem (victim));
		size = chunksize(victim);
...
#define MAX_ITERS	10000
	...
	/* Split */
	  ...
	  if (__builtin_expect (fwd->bk != bck, 0))
		 {
			errstr = "vip_malloc(): corrupted unsorted chunks 2";
			goto errout;
		 }
	  remainder->bk = bck;
	  remainder->fd = fwd;
	 ...
	 */
		...
		printf("<vipzone-glibc> _int_vip_malloc(): calling sYSVIPMALLOc(), we need more space from kernel!\n");
		void *p = sYSVIPMALLOc(nb, vip_flags, av); //vipzone
		if (p != NULL && __builtin_expect (perturb_byte, 0))
	alloc_perturb (p, bytes);
		return p;
	 }
