...
static void*  _int_vip_malloc(size_t, size_t); //vipzone
...
//vipzone -- this is more or less the 'public' handle for vip_malloc
//vipzone: we have made vip_malloc() routines as simple as humanly possible to avoid using sbrk() at all, or any unnecessary complications for our functionality. most of the stuff regular glibc malloc() does is for performance.
void*
public_vIPMALLOc(size_t bytes, size_t vip_flags)
{
	printf("<glibc-vipzone> vip_malloc(): request for %lu bytes, vip_flags = %lu\n",
		(long unsigned int)bytes, (long unsigned int)vip_flags);
	
	return _int_vip_malloc(bytes, vip_flags); //vipzone: we're just a wrapper for internal implementation which is "flexible"
}
...
//vipzone: this is the core vip_malloc() implementation. SIMPLE AS POSSIBLE!!! only uses vip_mmap, since we don't have a vip_sbrk() (yet).
static void*
_int_vip_malloc(size_t bytes, size_t vip_flags)
{
	void* p = (void *)(syscall(NR_vip_mmap, NULL, bytes, (PROT_READ | PROT_WRITE | vip_flags), (MAP_PRIVATE | MAP_ANONYMOUS), -1, 0));

	if (p == MAP_FAILED) {
		printf("<glibc-vipzone> _int_vip_malloc(): vip_mmap syscall FAILED!\n");
		return NULL;
	} else {
		printf("<glibc-vipzone> _int_vip_malloc(): vip_mmap syscall SUCCEEDED!\n");
		return p;
	}
}
